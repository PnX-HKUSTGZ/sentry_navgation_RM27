...
namespace gazebo
{
    ...
    void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf)
    {
        ...
        // RM27 navigation consumes PointCloud2 on /livox/lidar. Keep the
        // Livox CustomMsg available on a side topic for debugging.
        cloud2_pub = node_->create_publisher<sensor_msgs::msg::PointCloud2>(curr_scan_topic, 10);
        custom_pub = node_->create_publisher<livox_ros_driver2::msg::CustomMsg>(curr_scan_topic + "/custom", 10);
        ...
        if (maxPointSize == 0)
        {
            RCLCPP_ERROR(rclcpp::get_logger("LivoxPointsPlugin"),
                         "scan mode csv parsed to zero points, aborting Livox sensor init to avoid divide-by-zero");
            return;
        }
