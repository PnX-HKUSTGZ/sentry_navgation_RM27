...
void DynamicLayer::update(int width, int height, double resolution,
                          const Eigen::Vector2d &origin,
                          const std::vector<uint8_t> &mask,
                          double inflation_radius, double max_distance,
                          double min_distance, bool clamp_distance,
                          InterpolationMode interpolation,
                          FieldBuildStats *stats) {
  rebuild(width, height, resolution, origin, mask, inflation_radius,
          max_distance, min_distance, clamp_distance, interpolation, stats);
}

void DynamicLayer::rebuild(int width, int height, double resolution,
                           const Eigen::Vector2d &origin,
                           const std::vector<uint8_t> &mask,
                           double inflation_radius, double max_distance,
                           double min_distance, bool clamp_distance,
  ...
  for (size_t i = 0; i < expected; ++i) {
    double raw = 0.0;
    if (mask[i] == 1U) {
      raw = (dist_sq_pos[i] >= 1.0e19) ? kFarDistance
                                       : std::sqrt(dist_sq_pos[i]) * resolution;
    } else {
      raw = (dist_sq_neg[i] >= 1.0e19)
                ? -kFarDistance
                : -std::sqrt(dist_sq_neg[i]) * resolution;
    }
    const double inflated = raw - inflation_radius;
    dist_m[i] = std::isfinite(inflated)
                    ? inflated
                    : ((mask[i] == 1U) ? kFarDistance : -kFarDistance);
    if (clamp_distance) {
...
bool DynamicLayer::isValid() const {
  ...
  return width_ > 1 && height_ > 1 && resolution_ > 0.0 && !dist_m_.empty() &&
         dist_m_.size() == expected;
}

bool DynamicLayer::matchesGeometry(int width, int height, double resolution,
...
void DynamicLayer::evaluate(const Eigen::Vector3d &pos, double &dist,
  ...
  double fx = 0.0;
  double fy = 0.0;
  if (!interpolationCoordinate(pos.x(), origin_.x(), resolution_, width_, ix,
                               fx) ||
      !interpolationCoordinate(pos.y(), origin_.y(), resolution_, height_, iy,
                               fy)) {
    dist = max_distance_;
    grad.setZero();
    return;
  ...
  if (interpolation_ == InterpolationMode::QUADRATIC && ix >= 1 && iy >= 1 &&
      ix + 1 < width_ && iy + 1 < height_) {
  ...
  const size_t idx10 =
      static_cast<size_t>(iy) * w + static_cast<size_t>(ix + 1);
  const size_t idx01 =
      static_cast<size_t>(iy + 1) * w + static_cast<size_t>(ix);
  const size_t idx11 =
      static_cast<size_t>(iy + 1) * w + static_cast<size_t>(ix + 1);
