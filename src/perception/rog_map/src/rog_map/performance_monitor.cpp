...
void writeCsvLine(std::ofstream &stream,
...
PerformanceMonitor::ScopedTimer::ScopedTimer(PerformanceMonitor *monitor,
                                             double RuntimeStats::*field)
...
void PerformanceMonitor::configure(const PerformanceConfig &config) {
  ...
  if (detailedCsvEnabled()) {
    detailed_csv_.open(config_.detailed_csv_path,
                       std::ios::out | std::ios::trunc);
    if (!detailed_csv_.is_open()) {
      std::cerr << "[ROGMapPerf] failed to open detailed_csv_path: "
                << config_.detailed_csv_path << std::endl;
    } else {
      writeDetailedHeader();
    }
  }
  if (summaryCsvEnabled()) {
    summary_csv_.open(config_.summary_csv_path,
                      std::ios::out | std::ios::trunc);
    if (!summary_csv_.is_open()) {
      std::cerr << "[ROGMapPerf] failed to open summary_csv_path: "
                << config_.summary_csv_path << std::endl;
    } else {
      writeSummaryHeader();
    }
  }
...
void PerformanceMonitor::recordCloudCallback(double stamp, double points,
...
void PerformanceMonitor::recordValidCloud(double odom_age_ms) {
  ...
  const double stamp =
      last_cloud_stamp_ > 0.0 ? last_cloud_stamp_ : last_odom_stamp_;
...
void PerformanceMonitor::fillInputStats(RuntimeStats &stats) {
  ...
  stats.cloud_callback_hz =
      hzFrom(cloud_callback_count_, first_cloud_stamp_, last_cloud_stamp_);
  stats.cloud_msg_points = last_cloud_points_;
  stats.cloud_msg_points_avg = cloud_callback_count_ > 0.0
                                   ? cloud_points_sum_ / cloud_callback_count_
                                   : 0.0;
  ...
  stats.valid_update_hz = hzFrom(valid_cloud_count_, first_valid_update_stamp_,
                                 last_valid_update_stamp_);
  ...
  stats.odom_hz =
      hzFrom(odom_received_count_, first_odom_stamp_, last_odom_stamp_);
...
void PerformanceMonitor::addElapsed(double RuntimeStats::*field,
                                    double elapsed_ms) {
  if (!field) {
    return;
  }
...
void PerformanceMonitor::writeDetailedHeader() {
  writeCsvLine(detailed_csv_, {"run_id",
                               "scenario",
                               "variant",
                               "stamp_ros",
                               "stamp_steady_ns",
                               ...
                               "raycast_endpoint_dedup_time_ms",
                               "raycast_unique_endpoint_count",
                               "raycast_endpoint_dedup_count",
                               "prob_update_time_ms",
                               "decay_time_ms",
                               "projection_time_ms",
                               "field_time_ms",
                               ...
                               "reason_empty_column",
                               "reason_clearance_dropout_hold",
                               "reason_clearance_bounded_hole_fill",
                               "reason_thin_surface",
...
void PerformanceMonitor::writeSummaryHeader() {
  writeCsvLine(summary_csv_,
               {"run_id", "scenario", "variant", "window_start_stamp",
                "window_duration_sec", "cloud_callback_hz", "map_update_hz",
                "field_update_hz", "last_total_update_time_ms",
                "last_raycast_time_ms", "last_projection_time_ms",
                "last_field_time_ms", "last_free_count", "last_passable_count",
                "last_occupied_count", "last_unknown_count"});
}

void PerformanceMonitor::writeDetailedRow(const RuntimeStats &s) {
  writeCsvLine(detailed_csv_, {sanitize(config_.run_id),
                               ...
                               num(s.raycast_endpoint_dedup_time),
                               num(s.raycast_unique_endpoint_count),
                               num(s.raycast_endpoint_dedup_count),
                               ...
                               num(s.projection_clearance_dropout_hold_count),
                               num(s.projection_clearance_hole_fill_count),
...
void PerformanceMonitor::maybeWriteSummary(double stamp) {
  ...
  const double period =
      config_.summary_rate > 0.0 ? 1.0 / config_.summary_rate : 1.0;
  const double duration = std::max(0.0, stamp - window_.start_stamp);
  if (duration + 1.0e-9 < period) {
    return;
  }
  const double cloud_hz =
      duration > 1.0e-6 ? window_.cloud_callbacks / duration : 0.0;
  const double valid_hz =
      duration > 1.0e-6 ? window_.valid_updates / duration : 0.0;
  const double field_hz =
      duration > 1.0e-6 ? window_.field_updates / duration : 0.0;

  if (summaryCsvEnabled() && summary_csv_.is_open()) {
    writeCsvLine(summary_csv_,
                 {sanitize(config_.run_id), sanitize(config_.scenario),
                  sanitize(config_.variant), num(window_.start_stamp),
                  num(duration), num(cloud_hz), num(valid_hz), num(field_hz),
                  num(stats_.total_update_time), num(stats_.raycast_time),
                  num(stats_.projection_total_time), num(stats_.field_time),
                  num(stats_.free_count), num(stats_.passable_count),
                  num(stats_.occupied_count), num(stats_.unknown_count)});
    flushIfNeeded(summary_csv_, summary_csv_rows_);
  }

  if (printEnabled()) {
    std::cerr
        << "[ROGMapPerf] win=" << std::fixed << std::setprecision(2) << duration
        ...
        << " (parallel=" << stats_.raycast_parallel_time
        << " merge=" << stats_.raycast_merge_time
        << " endpoint_dedup=" << stats_.raycast_endpoint_dedup_time << ")"
        << " prob=" << stats_.prob_update_time
        << " projection=" << stats_.projection_total_time
        << " field=" << stats_.field_time
        << " snapshot=" << stats_.query_refresh_time << '\n'
        << "  input_age_ms: cloud_to_callback=" << stats_.cloud_queue_delay_ms
        << " cloud_odom_stamp_delta=" << stats_.odom_age_ms << '\n'
        << "  rays: input=" << stats_.raycast_used_point_count
        << " unique_endpoints=" << stats_.raycast_unique_endpoint_count
        << " duplicate_endpoints=" << stats_.raycast_endpoint_dedup_count
        << '\n'
        ...
        << " dropout_hold=" << stats_.projection_clearance_dropout_hold_count
        << " bounded_fill=" << stats_.projection_clearance_hole_fill_count
...
void PerformanceMonitor::flushIfNeeded(std::ofstream &stream, int &row_count) {
  ++row_count;
  if (config_.csv_flush_every_n > 0 &&
      row_count % config_.csv_flush_every_n == 0) {
    stream.flush();
  }
}
