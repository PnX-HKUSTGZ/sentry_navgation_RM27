  ...
  const PointCloud & cloud, const Pose & sensor_pose, const Vec3f & map_center_pos)
{
  ...
  if (raycast_data_.batch_update_counter >= cfg_.batch_update_size) {
    ...
    const auto prob_update_start = std::chrono::steady_clock::now();
    probabilisticMapFromCache();
    runtime_stats_.prob_update_time = elapsedMs(prob_update_start);
    map_empty_ = false;
  }
  ...
  runtime_stats_.total_update_time = elapsedMs(update_start);
...
void ProbMap::raycastProcessParallel(const PointCloud & input_cloud, const Vec3f & cur_odom)
{
  ...
  vec_E<Vec3i> ray_endpoint_ids;
  vec_E<Vec3i> hit_ids;
  raycasting_cloud.reserve(input_cloud.size());
  ray_endpoint_ids.reserve(input_cloud.size());
  ...
  for (const auto & pcl_p : input_cloud) {
    ...
    posToGlobalIndex(p, pt_id_g);
    raycasting_cloud.push_back(p);
    ray_endpoint_ids.push_back(pt_id_g);
    runtime_stats_.raycast_used_point_count += 1.0;
  ...
  // Dense organized scans contain many endpoints in the same ROG voxel. All
  // hit voxels remain in hit_ids; only duplicate free-space walks are removed.
  // This is conservative because omitting a duplicate ray can leave a voxel
  // unknown, but can never create free space or discard an occupied endpoint.
  const auto endpoint_dedup_start = std::chrono::steady_clock::now();
  std::vector<size_t> endpoint_order(ray_endpoint_ids.size());
  for (size_t i = 0; i < endpoint_order.size(); ++i) {
    endpoint_order[i] = i;
  }
  std::sort(endpoint_order.begin(), endpoint_order.end(),
    [&ray_endpoint_ids](size_t lhs, size_t rhs) {
      return vec3iLess(ray_endpoint_ids[lhs], ray_endpoint_ids[rhs]);
    });
  vec_E<Vec3f> unique_raycasting_cloud;
  unique_raycasting_cloud.reserve(raycasting_cloud.size());
  Vec3i previous_endpoint = Vec3i::Zero();
  bool have_previous_endpoint = false;
  for (const size_t index : endpoint_order) {
    const Vec3i & endpoint = ray_endpoint_ids[index];
    if (have_previous_endpoint && endpoint == previous_endpoint) {
      continue;
    }
    unique_raycasting_cloud.push_back(raycasting_cloud[index]);
    previous_endpoint = endpoint;
    have_previous_endpoint = true;
  }
  runtime_stats_.raycast_unique_endpoint_count =
    static_cast<double>(unique_raycasting_cloud.size());
  runtime_stats_.raycast_endpoint_dedup_count =
    static_cast<double>(raycasting_cloud.size() - unique_raycasting_cloud.size());
  runtime_stats_.raycast_endpoint_dedup_time = elapsedMs(endpoint_dedup_start);
  raycasting_cloud.swap(unique_raycasting_cloud);
  ...
  if (occupancy_buffer_.size() > static_cast<size_t>(std::numeric_limits<int>::max())) {
    throw std::length_error("ROG map is too large for integer voxel hash IDs");
  }
  const size_t map_voxel_count = occupancy_buffer_.size();
  for (auto & local : local_buffers) {
    local.miss_flags.assign(map_voxel_count, uint8_t{0});
    local.miss_hash_ids.reserve(raycasting_cloud.size() * 4U);
  }
...
#pragma omp for schedule(dynamic, 32)
    for (int i = 0; i < static_cast<int>(raycasting_cloud.size()); ++i) {
      ...
      while (local_raycaster.step(ray_pt)) {
        ...
        if (local.miss_flags[static_cast<size_t>(hash_id)] == uint8_t{0}) {
          local.miss_flags[static_cast<size_t>(hash_id)] = uint8_t{1};
          local.miss_hash_ids.push_back(hash_id);
        }
      }
    }
  }
  ...
  std::vector<uint8_t> merged_miss_flags(map_voxel_count, uint8_t{0});
  std::vector<int> merged_miss_hash_ids;
  size_t local_unique_miss_count = 0;
  for (const auto & local : local_buffers) {
    local_unique_miss_count += local.miss_hash_ids.size();
  }
  merged_miss_hash_ids.reserve(local_unique_miss_count);
  for (const auto & local : local_buffers) {
    for (const int hash_id : local.miss_hash_ids) {
      auto & is_miss = merged_miss_flags[static_cast<size_t>(hash_id)];
      if (is_miss == uint8_t{0}) {
        is_miss = uint8_t{1};
        merged_miss_hash_ids.push_back(hash_id);
      }
    }
  }

  uniqueVec3i(hit_ids);
  std::vector<uint8_t> hit_flags(map_voxel_count, uint8_t{0});
  for (const auto & id_g : hit_ids) {
    hit_flags[static_cast<size_t>(getHashIndexFromGlobalIndex(id_g))] = uint8_t{1};
    insertUpdateCandidate(id_g, true);
  }
  for (const int hash_id : merged_miss_hash_ids) {
    if (hit_flags[static_cast<size_t>(hash_id)] == uint8_t{0}) {
      Vec3i id_g;
