...
- raycast 可配置并行线程；当前仿真与实车 MINCO YAML 均启用 4 线程，实际线程数仍需按 CPU P99 数据调节。
...
| `raycasting.parallel_enable` | 是否并行 raycast；优先于兼容用的 `performance.parallel_raycast_enable` |
| `raycasting.num_threads` | raycast 并行线程数；优先于兼容用的 `performance.raycast_num_threads` |
