...
struct RuntimeStats {
  ...
  double raycast_endpoint_dedup_time{0.0};
  double raycast_unique_endpoint_count{0.0};
  double raycast_endpoint_dedup_count{0.0};
  ...
  double projection_clearance_dropout_hold_count{0.0};
  double projection_clearance_hole_fill_count{0.0};
...
public:
  ...
  bool summaryCsvEnabled() const {
    return config_.enable && config_.summary_csv_enable;
  }
  ...
  ScopedTimer scoped(double RuntimeStats::*field) {
    return ScopedTimer(this, field);
  }

  void recordCloudCallback(double stamp, double points, double queue_delay_ms,
