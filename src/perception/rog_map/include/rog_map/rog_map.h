...
public:
  ...
  bool getNearestCellIs(const GridType &target_type, const Vec3f &start_pos,
                        Vec3f &nearest_pt, const double &max_dis) const {
    return findNearestCellThat(IS, target_type, start_pos, nearest_pt, max_dis);
  }

  bool getNearestCellNot(const GridType &target_type, const Vec3f &start_pos,
                         Vec3f &nearest_pt, const double &max_dis) const {
    return findNearestCellThat(NOT, target_type, start_pos, nearest_pt,
                               max_dis);
  }

  bool getNearestInfCellIs(const GridType &target_type, const Vec3f &start_pos,
                           Vec3f &nearest_pt, const double &max_dis) const {
    return findNearestInfCellThat(IS, target_type, start_pos, nearest_pt,
                                  max_dis);
  }

  bool getNearestInfCellNot(const GridType &target_type, const Vec3f &start_pos,
                            Vec3f &nearest_pt, const double &max_dis) const {
    return findNearestInfCellThat(NOT, target_type, start_pos, nearest_pt,
                                  max_dis);
  }
...
protected:
  ...
  void updateMapInternal(
      ...
      double sensor_stamp = std::numeric_limits<double>::quiet_NaN());
