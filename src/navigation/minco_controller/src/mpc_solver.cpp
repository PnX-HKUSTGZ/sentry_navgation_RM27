...
void MpcSolver::reset() noexcept {
  last_u_global_.setZero();
  has_last_u_ = false;
}

void MpcSolver::buildStepModel(double /*yaw_ref*/, Eigen::Matrix3d &A,
...
bool MpcSolver::buildCondensedQP(const State &curr,
                                 ...
                                 Eigen::MatrixXd &A_con, Eigen::VectorXd &lbA,
                                 Eigen::VectorXd &ubA) {
  ...
  // 这里 A_con * U 直接表示 (v_k - v_{k-1})，再用 bounds 限制到 [a_min*dt,
  // a_max*dt]
  for (int k = 0; k < N; ++k) {
    for (int axis = 0; axis < 3; ++axis) {
      ...
      if (k == 0) {
        ...
        const double last =
            (axis == 0 ? last_vx : (axis == 1 ? last_vy : last_w));
        lbA(row) =
            (axis == 0 ? config_.ax_min
                       : (axis == 1 ? config_.ay_min : config_.alpha_min)) *
                config_.dt +
            last;
        ubA(row) =
            (axis == 0 ? config_.ax_max
                       : (axis == 1 ? config_.ay_max : config_.alpha_max)) *
                config_.dt +
            last;
      } else {
...
bool MpcSolver::solve(const State &curr,
                      const std::vector<ReferencePoint> &ref_traj,
                      Control &out_u, std::vector<State> *out_pred) {
  out_u = Control{};
  if (out_pred) {
    out_pred->clear();
  }
  const auto fail_closed = [this, out_pred]() {
    reset();
    if (out_pred) {
      out_pred->clear();
    }
  ...
  const int N = config_.horizon;
  if (N <= 0 || !std::isfinite(config_.dt) || config_.dt <= 0.0 ||
      !config_.Q.allFinite() || !config_.R.allFinite() ||
      (config_.Q.array() < 0.0).any() || (config_.R.array() <= 0.0).any() ||
      !std::isfinite(config_.q_along) || !std::isfinite(config_.q_cross) ||
      config_.q_along < 0.0 || config_.q_cross < 0.0 ||
      !std::isfinite(config_.max_planar_speed) ||
      config_.max_planar_speed <= 0.0 ||
      !std::isfinite(config_.eps_regularization) ||
      config_.eps_regularization < 0.0 ||
      config_.max_working_set_recalculations <= 0) {
    return fail_closed();
  }
  if (static_cast<int>(ref_traj.size()) < N) {
    return fail_closed();
  ...
  for (int i = 0; i < N; ++i) {
    const auto &ref = ref_traj[static_cast<size_t>(i)];
    if (!ref.pos.allFinite() || !ref.vel.allFinite() ||
        !std::isfinite(ref.yaw) || !std::isfinite(ref.yaw_rate)) {
      return fail_closed();
    }
  }
  ...
  if (nV <= 0 || H.rows() != nV || H.cols() != nV || lb.size() != nV ||
      ub.size() != nV || !H.allFinite() || !g.allFinite() || !lb.allFinite() ||
      !ub.allFinite() || !(lb.array() <= ub.array()).all()) {
    return fail_closed();
  }
  if (nC < 0 || ubA.size() != nC || A_con.rows() != nC ||
      (nC > 0 &&
       (A_con.cols() != nV || !A_con.allFinite() || !lbA.allFinite() ||
        !ubA.allFinite() || !(lbA.array() <= ubA.array()).all()))) {
    return fail_closed();
  }

  std::vector<qpOASES::real_t> H_qp(
      static_cast<size_t>(nV) * static_cast<size_t>(nV), 0.0);
  for (int i = 0; i < nV; ++i) {
    for (int j = 0; j < nV; ++j) {
      H_qp[static_cast<size_t>(i) * static_cast<size_t>(nV) +
           static_cast<size_t>(j)] = static_cast<qpOASES::real_t>(H(i, j));
    }
  }
  ...
  if (nC > 0) {
    ...
    for (int i = 0; i < nC; ++i) {
      for (int j = 0; j < nV; ++j) {
        A_qp[static_cast<size_t>(i) * static_cast<size_t>(nV) +
  ...
  const qpOASES::returnValue ret =
      qp.init(H_qp.data(), g_qp.data(), A_ptr, lb_qp.data(), ub_qp.data(),
              lbA_ptr, ubA_ptr, nWSR);

  if (ret != qpOASES::SUCCESSFUL_RETURN) {
    if (ret == qpOASES::RET_MAX_NWSR_REACHED) {
      std::cout << color_text::RED << "[MpcSolver] qpOASES Max NWSR Reached!"
                << color_text::RESET << std::endl;
    } else if (ret == qpOASES::RET_INIT_FAILED_INFEASIBILITY) {
      std::cout << color_text::RED << "[MpcSolver] qpOASES Infeasible!"
                << color_text::RESET << std::endl;
    } else {
      std::cout << color_text::RED
                << "[MpcSolver] qpOASES Error Code: " << static_cast<int>(ret)
                << color_text::RESET << std::endl;
    }
    return fail_closed();
  }
  ...
  if (qp.getPrimalSolution(xOpt.data()) != qpOASES::SUCCESSFUL_RETURN) {
    return fail_closed();
  }

  constexpr double kSolutionTolerance = 1.0e-6;
  for (int i = 0; i < nV; ++i) {
    const double value = static_cast<double>(xOpt[static_cast<size_t>(i)]);
    if (!std::isfinite(value) || value < lb(i) - kSolutionTolerance ||
        value > ub(i) + kSolutionTolerance) {
      return fail_closed();
    }
    xOpt[static_cast<size_t>(i)] =
        static_cast<qpOASES::real_t>(std::clamp(value, lb(i), ub(i)));
  }
  for (int row = 0; row < nC; ++row) {
    double value = 0.0;
    for (int col = 0; col < nV; ++col) {
      value +=
          A_con(row, col) * static_cast<double>(xOpt[static_cast<size_t>(col)]);
    }
    if (!std::isfinite(value) || value < lbA(row) - kSolutionTolerance ||
        value > ubA(row) + kSolutionTolerance) {
      return fail_closed();
    }
  }

  // The QP uses independent vx/vy bounds, whose corners permit sqrt(2) times
  // the intended trajectory speed. Project every predicted command onto the
  // shared planar-speed disk so prediction, warm acceleration state, and the
  // command sent to the robot obey the same braking-distance assumption.
  for (int i = 0; i < N; ++i) {
    auto &vx = xOpt[static_cast<size_t>(3 * i + 0)];
    auto &vy = xOpt[static_cast<size_t>(3 * i + 1)];
    const double speed = std::hypot(static_cast<double>(vx),
                                    static_cast<double>(vy));
    if (!std::isfinite(speed)) {
      return fail_closed();
    }
    if (speed > config_.max_planar_speed) {
      const double scale = config_.max_planar_speed / speed;
      vx = static_cast<qpOASES::real_t>(static_cast<double>(vx) * scale);
      vy = static_cast<qpOASES::real_t>(static_cast<double>(vy) * scale);
    }
  }
  ...
  if (out_pred) {
    ...
    for (int i = 0; i < N; ++i) {
      const double ux =
          static_cast<double>(xOpt[static_cast<size_t>(3 * i + 0)]);
      const double uy =
          static_cast<double>(xOpt[static_cast<size_t>(3 * i + 1)]);
      const double uw =
          static_cast<double>(xOpt[static_cast<size_t>(3 * i + 2)]);
      ...
      s.y += dy;
      s.yaw += dyaw;
      if (!std::isfinite(s.x) || !std::isfinite(s.y) || !std::isfinite(s.yaw)) {
        return fail_closed();
      }
  ...
  // 取第一个控制量 u0 (global)
  const Eigen::Vector3d u0_global(static_cast<double>(xOpt[0]),
                                  static_cast<double>(xOpt[1]),
                                  static_cast<double>(xOpt[2]));
  if (!u0_global.allFinite()) {
    return fail_closed();
  }
