...
struct MPCConfig {
  ...
  // Q.z is the yaw weight. q_along/q_cross are rotated into the global XY
  // frame at each horizon step; Q.x/Q.y are retained for message/config ABI.
  ...
  double vy_min{-1.0};
  double vy_max{1.0};
  // vx/vy are box constraints for the QP. This radial limit is the actual
  // planar-speed contract shared with the trajectory optimizer.
  double max_planar_speed{1.0};
