...
public:
  ...
  void applyGravityCompensation(const nav_msgs::msg::Odometry::SharedPtr &odom,
                                double &vx, double &vy);
...
private:
  ...
  void configureMpcPerfLogging(
      const rclcpp_lifecycle::LifecycleNode::SharedPtr &node);
  geometry_msgs::msg::TwistStamped failClosedCommand(const rclcpp::Time &stamp,
                                                     const std::string &reason);
  void resetSolverState();
  bool
  snapshotFreshInputs(const rclcpp::Time &now,
                      const std::chrono::steady_clock::time_point &steady_now,
                      nav_msgs::msg::Odometry::SharedPtr &odom,
                      uint64_t &trajectory_generation,
  ...
  void
  extractGlobalVelocityAndYaw(const nav_msgs::msg::Odometry::SharedPtr &odom,
                              double &vx_global, double &vy_global,
                              double &omega_global, double &yaw_global) const;
  ...
  inline static Eigen::Vector2d interpolate(const Eigen::Vector2d &v1,
  ...
  // --- Visualization ---
  void publishVisualization(const std::vector<State> &pred_path,
                            const State &curr_state);

  // === ROS 2 Interfaces (Publishers, Subscribers, Timers) ===
  rclcpp::Subscription<ros_interfaces::msg::MpcPositionCommand>::SharedPtr
      opt_path_sub_;
  ...
  std::string cmd_vel_mpc_topic_{"cmd_vel_mpc"};
  ...
  ros_interfaces::msg::MpcPositionCommand::SharedPtr pending_opt_path_;
  nav_msgs::msg::Odometry::SharedPtr latest_odom_;
  std::chrono::steady_clock::time_point last_opt_path_rx_time_{};
  std::chrono::steady_clock::time_point pending_opt_path_rx_time_{};
  std::chrono::steady_clock::time_point last_odom_rx_time_{};
  bool has_opt_path_rx_time_{false};
  bool has_pending_opt_path_rx_time_{false};
  bool has_odom_rx_time_{false};
  bool blocked_{true};
  uint64_t trajectory_generation_{0};
  TrajectorySessionGate trajectory_session_gate_;

  // --- Reference Tracking State ---
  // Track reference index over time when trajectory is not updated to avoid
  // jumping back to start.
  mutable double tracked_spatial_idx_{0.0};
  ...
  double trajectory_timeout_{1.5};
