  ...
  std::function<bool()> cancel_checker)
{
  ...
  if (path.size() == 1u) {
    // A non-zero goal tolerance can make the start cell a valid search
    // result. Keep the caller's goal-to-start ordering and add the checked,
    // traversable goal cell so downstream local planning receives a real
    // short segment instead of retrying this successful search at 20 Hz.
    path.insert(path.begin(), Coordinates(static_cast<float>(goal_x),
