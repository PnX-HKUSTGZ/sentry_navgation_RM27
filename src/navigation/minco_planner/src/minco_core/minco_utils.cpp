...
bool quaternionToYawChecked(const geometry_msgs::msg::Quaternion & q, double & yaw)
{
  ...
  const double sin_yaw = 2.0 * (q.w * q.z + q.x * q.y) / norm_sq;
  const double cos_yaw = 1.0 - 2.0 * (q.y * q.y + q.z * q.z) / norm_sq;
...
{
  double yaw = 0.0;
  return quaternionToYawChecked(q, yaw) ? yaw : 0.0;
}
  ...
  double fallback_yaw)
{
  ...
  if (steps <= 0 || backup_traj.empty()) {
    return traj_msg;
  }
  traj_msg.cmds.resize(static_cast<size_t>(steps));
...
void publishBackupTrajectory(const traj_opt::Trajectory & backup_traj,
  const rclcpp::Publisher<ros_interfaces::msg::MpcPositionCommand>::SharedPtr & authoritative_pub,
  const rclcpp::Publisher<ros_interfaces::msg::MpcPositionCommand>::SharedPtr & debug_pub,
  uint32_t & trajectory_id_counter,
  const std_msgs::msg::Header & header,
  const builtin_interfaces::msg::Time & planning_stamp,
  int steps,
  ...
  double fallback_yaw)
{
  if (!authoritative_pub || steps <= 0) {
    return;
  }

  const auto command = makeBackupTrajectoryCommand(
    backup_traj, ++trajectory_id_counter, header, planning_stamp, steps, t_step, fallback_yaw);
  authoritative_pub->publish(command);
  if (debug_pub) {
    // The backup topic mirrors the authoritative BLOCK command for inspection;
    // controllers remain subscribed only to the authoritative trajectory topic.
    debug_pub->publish(command);
  }
}
  ...
  traj_opt::Trajectory & yaw_traj)
{
  position_traj.clear();
  yaw_traj.clear();
  if (!escape_vel.allFinite() || !std::isfinite(current_yaw) ||
  ...
  cMat(2, 4) = 0.0;
  position_traj.emplace_back(duration, cMat);
