...
: base_(std::move(base)), clearance_radius_(clearance_radius)
{
  if (!base_) {
    throw std::invalid_argument("StaticObstacleClearanceQuery requires a base query");
  }
  buildOverlay();
}
...
void StaticObstacleClearanceQuery::buildOverlay()
{
  ...
  if (!base_->copyValues(base_values) || base_values.size() != cell_count) {
    throw std::invalid_argument("StaticObstacleClearanceQuery could not snapshot the base map");
  }
  ...
  for (unsigned int my = 0U; my < ny; ++my) {
    for (unsigned int mx = 0U; mx < nx; ++mx) {
      ...
      if (base_values[source_index] != nav2_costmap_2d::LETHAL_OBSTACLE) {
        continue;
      }
